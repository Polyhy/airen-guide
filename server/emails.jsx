...
Emails.BeforeVote = React.createClass({
	propTypes: {
		emailForm: PropTypes.shape({
			teamName: PropTypes.string.isRequired,
			userName: PropTypes.string.isRequired,
			voteLink: PropTypes.array.isRequired,
			voteInfo: PropTypes.object.isRequired,
			url: PropTypes.string.isRequired
		}).isRequired
	},
	renderVote: function(){
		return this.props.voteLink.map(
				link=><p>{link.menu+" : "}<a href={link.url}>{link.url}</a></p>
		);
	},
	render: function(){
		return (
					...
					<tr>
						<td style={{borderLeft:"1px solid #d5d5d5",borderRight:"1px solid #d5d5d5"}}>
							<table align="center" border="0" width="628" cellPadding="0" cellSpacing="2"
								...
								<tr height="60px">
									<td width="60px"></td>
									<td width="510px;"><p><b style={{color:"#545454"}}>{this.props.emailForm.userName}</b>님, 무엇을 먹으실건가요?</p>
										<hr/>
									</td>
									<td width="60px;"></td>
								</tr>
								<tr>
									<td width="60px;"></td>
									<td width="510px;">
										<p>{this.props.emailForm.teamName}에서 투표를 진행하고 있습니다.</p>
										<br/>
										<p>{this.props.voteInfo.end+"까지"}</p>
										<p>{this.props.emailForm.teamName}에서 투표를 하거나</p>
										<br/>
										<p>아래의 링크를 통해 투표를 진행 해 주세요</p>
										{this.renderVote()}
										<br/>
										<p><a href={this.props.emailForm.url}>{this.props.emailForm.url}</a></p>
									</td>
